...
public class PackageDaoImpl implements PackageDao {
    ...
    private static final String SQL_CREATE =
            "INSERT INTO package(id, max_product, max_member, price, period_validity, state, created_at, created_by) " +
                    "values(%d, %d, %d, %16.4f, %d, %d, %d, %d)";
    private static final String SQL_UPDATE =
            "UPDATE package SET state = %d, updated_at = %d, updated_by = %d WHERE id = %d";
    private static final String SQL_SELECT_BY_ID =
            "SELECT * FROM package WHERE id = %d";
    private static final String SQL_SELECT_BY_STATE =
            "SELECT * FROM package WHERE state = %d";
    private static final String SQL_SELECT =
            "SELECT * FROM package";

    @Override
    public void create(PackageEntity entity) throws IOException, DBServiceException {
        String query = String.format(SQL_CREATE, entity.getId(), entity.getMaxProduct(),
