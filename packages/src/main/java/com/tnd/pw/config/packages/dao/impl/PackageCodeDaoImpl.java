...
public class PackageCodeDaoImpl implements PackageCodeDao {
    ...
    private static final String SQL_CREATE =
            "INSERT INTO package_code(id, package_id, expire_time, state, created_at, created_by, updated_at, updated_by) " +
    ...
    private static final String SQL_SELECT_BY_DURATION_TIME =
            "SELECT * FROM package_code WHERE created_at BETWEEN %d AND %d ORDER BY created_at ASC";
    ...
    @Override
    public List<PackageCodeEntity> get(Long startTime, Long endTime) throws DBServiceException, PackageCodeNotFoundException {
        String query = String.format(SQL_SELECT_BY_DURATION_TIME, startTime, endTime);
        List<PackageCodeEntity> entities = dataHelper.querySQL(query, PackageCodeEntity.class);
        if(CollectionUtils.isEmpty(entities)) {
            throw new PackageCodeNotFoundException();
        }
